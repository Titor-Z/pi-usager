...
> **依赖关系（关键）**：pi-usager **强依赖** `@foolsecret/pi-pricer` 提供价格数据。未安装时，用量统计照常，但**费用无法估算**（状态栏显示 `价格未知`，并在启动时提示安装）。这是 v2.0.0 起的设计选择 —— 价表由专职模块统一维护，插件不再各自硬编码，避免多扩展口径不一致。
