...
| 余额分档色 | < ¥0.50 红（告急，可配置）→ < ¥1.00 黄（提醒，可配置）→ 其余主题蓝（充裕）；`/usage config` 可改两条线 |
| `CH` 分色 | 缓存命中率：<90% 常态色 → 90~95% 主题蓝（良好）→ ≥95% 紫（优秀） |
| `·限时5折` | 当前计价档位标记（限时折扣等） |
...
| `/usage config` | 配置菜单：厂商凭证 / 余额校准间隔 / 自定义计价 / HUD 状态栏开关 / HUD 布局（双行⭐/单行）/ 余额颜色阈值 / 清除凭证 / 查看当前配置 |
| 自定义计价 | 厂商调价不再等插件更新：`/usage config → 自定义计价` 录入三价（缓存命中/未命中/输出，¥/M）+ 闹钟式时段表（生效日 + 小时区间，支持跨午夜），完全接管该模型计费，立即生效 |

> `/usage config` 是唯一配置入口（凭证、校准、自定义计价、HUD、余额颜色）。配置存储于 `~/.pi/pi-usager.json`（含凭证与余额缓存，勿分享）。
